...
def _print_season_report(report: dict) -> None:
    ...
    print(f'  {label}: {report.get("gameCount", 0)} games · '
...
def main() -> None:
    ...
    for season_year in season_years:
        out_path = season_output_path(season_year, ROOT)
        print(f'\n--- Season {season_year} → {out_path.relative_to(ROOT)} ---')
        ...
        if args.skip_existing and out_path.is_file():
            ...
            for s in doc.get('seasons') or []:
                if s.get('warning'):
                    warnings.append(f'{s.get("seasonLabel")}: {s["warning"]}')
            continue
        ...
        if validation_errors:
            msg = f'{season_report["seasonLabel"]}: {len(validation_errors)} validation error(s)'
            warnings.append(msg)
            print(f'  Validation FAILED ({len(validation_errors)} errors)')
            for err in validation_errors[:5]:
                print(f'    - {err}')
            if not args.dry_run:
                raise SystemExit(f'Validation failed for {season_report["seasonLabel"]}')

        if season_report.get('warning'):
            warnings.append(f'{season_report["seasonLabel"]}: {season_report["warning"]}')
